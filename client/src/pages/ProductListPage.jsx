  ...
  const fetchProductdata = async () => {
    try {
      ...
      if (responseData.success) {
        setData((prev) => responseData.page === 1 ? responseData.data : [...prev, ...responseData.data])
        ...
        <div className=' min-h-[88vh] max-h-[88vh] overflow-y-scroll  grid gap-1 shadow-md scrollbarCustom bg-white py-2'>
          {
            DisplaySubCatory.map((s, index) => {
              ...
              return (
                ...
                `}
                >
                  <p className='text-xs text-center lg:text-left lg:text-base font-bold text-purple-900'>{s.name}</p>
                </Link>
              )
            })
          }
            ...
            <div className=' grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 p-4 gap-4 '>
                {
                  data.length === 0 && !loading ? (
                    <div className='col-span-full text-center py-10 text-neutral-500'>
                      <p className='font-semibold'>No products available for this selection.</p>
