...
const DisplayCartItem = ({close}) => {
  ...
  return (
    <section className='bg-neutral-900 fixed top-0 bottom-0 right-0 left-0 bg-opacity-70 z-50'>
        <div className='bg-white w-full lg:w-1/2 lg:max-w-none min-h-screen max-h-screen ml-auto'>
            ...
            <div className='min-h-[75vh] lg:min-h-[80vh] h-full max-h-[calc(100vh-150px)] bg-blue-50 p-2 flex flex-col gap-4'>
                {/***display items */}
                {
                    cartItem[0] ? (
                        <>
                                            <div className='flex items-center justify-between px-4 py-2 bg-red-100 text-red-500 rounded-full'>
                            ...
                            <div className='bg-white rounded-lg p-4 grid gap-5 overflow-auto'>
                                    {
                                        cartItem[0] && (
                                            cartItem.map((item,index)=>{
                                                const isAvailable = item?.productId && item?.productId?.stock > 0 && item?.productId?.publish === true
                                                return(
                                                    <div key={item?._id+"cartItemDisplay"} className='flex  w-full gap-4 relative'>
                                                        <div className='w-16 h-16 min-h-16 min-w-16 bg-red-500 border rounded'>
                                                            <img
                                                                src={getFirstImage(item?.productId?.image) || imageEmpty}
                                                        ...
                                                        <div className='w-full max-w-sm text-xs'>
                                                            ...
                                                            {!isAvailable && (
                                                                <p className='text-red-600 text-xs font-semibold mt-1'>
                                                                    {!item?.productId ? 'Product deleted' : item?.productId?.stock <= 0 ? 'Out of stock' : 'Unavailable'}
                                                                </p>
                                                            )}
                                                        </div>
                                                        <div>
                            ...
                            <div className='bg-white p-4'>
                                <h3 className='font-semibold text-red-700'>Bill details</h3>
