                ...
                <div className=' flex gap-4 md:gap-6 lg:gap-8 container mx-auto px-4 overflow-x-scroll scrollbar-none scroll-smooth' ref={containerRef}>
...

                    {
                        !loading && data.length === 0 ? (
                            <div className='w-full min-h-[180px] flex items-center justify-center rounded-lg border border-dashed border-gray-300 bg-white p-6'>
                                <p className='text-center text-sm text-gray-500'>No products available in this category yet.</p>
                            </div>
                        ) : (
                            data.map((p, index) => {
                                return (
